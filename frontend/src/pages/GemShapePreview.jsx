...
const GemShapePreview = () => {
  ...
  const handleUploadGem = async () => {
    ...
    try {
      ...
      navigate('/GemShapeResult', { state: { prediction: apiResponse.data, imageDataUrl } });
    } catch (err) {
      setLoading(false);
  ...
  return (
    ...
    <button className="upload-button-2" onClick={handleCaptureClick}>Retake Image</button>
    <button className="upload-button" onClick={handleUploadGem} disabled={loading}> {loading ? 'Uploading...' : 'Upload Gem'}</button>
</div>
